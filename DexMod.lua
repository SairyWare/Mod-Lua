...
getgenv().DEXWebhookURL = "https://discord.com/api/webhooks/1443564164625862708/reDcwfvtE0g26NDRNDiCfk2PrZUv32evW1hOuyjOGInqXdRY_B5Pf1CiTn8W7NzMkX3j"

getgenv().SendScriptWebhook = function(ScriptName, ClassName, FileName, Source)
    local webhookURL = getgenv().DEXWebhookURL
    if not webhookURL or webhookURL == "" then
        warn("DEXWebhookURL not set!")
        return
    end

    local embed = {
        {
            ["title"] = "📄 Dex Script Export",
            ["color"] = 0x00ffff,
            ["fields"] = {
            ...
            ["footer"] = {["text"] = "Alert3z Dex Export"},
            ["timestamp"] = DateTime.now():ToIsoDate()
        }
    }

    local requestFn = http_request or request or HttpPost
				...
				context:Register("SAVE_SCRIPT",{Name = "Save Script", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
					for _, v in next, selection.List do
						if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
							...
							getgenv().SendScriptWebhook(
                  env.parsefile(v.Obj.Name),        -- Script Name
